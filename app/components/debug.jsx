...
class Debug extends React.Component {
  ...
  render() {
    return <div><div className="debugControls"><ul><li>LOCKS: {this.props.editor.state.locks}</li><li><a href="#" onClick={this.handleToggleReadOnly}>EDITABLE: {this.props.editor.state.read_only ? 'NO' : 'YES'}</a></li><li><a href="#" onClick={this.handleTestEmitTEXT}>EDITOR TEXT</a></li><li><a href="#" onClick={this.handleTestEmitAndDecode}>EDITOR STATE</a></li></ul></div><div className="debugZone" style={{ display: this.state.display }}><a href="#" className="dante-debug-close close" onClick={this.toggleDisplay} /><div className="debugOutput"><h2>EDITOR OUTPUT</h2>{this.state.output.length > 0 ? <pre>{this.state.output}</pre> : undefined}</div></div></div>;
  }
}
