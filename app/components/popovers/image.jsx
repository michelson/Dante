...
class DanteImagePopover extends React.Component {
  ...
  render() {
    return <div ref="image_popover" className={`dante-popover popover--Aligntooltip popover--top popover--animated ${ this.state.show ? 'is-active' : undefined }`} style={{ top: this.state.position.top, left: this.state.position.left }}><div className='popover-inner'><ul className='dante-menu-buttons'>{this.state.buttons.map((item, i) => {
            return <DanteImagePopoverItem item={item} handleClick={this.handleClick} key={i} />;
          })}</ul></div><div className='popover-arrow' /></div>;
  }
}
