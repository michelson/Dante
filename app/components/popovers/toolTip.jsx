...
class DanteTooltip extends React.Component {
  ...
  render() {
    return <div id="dante-menu" ref="dante_menu" className={`dante-menu ${ this.displayActiveMenu() } ${ this.displayLinkMode() }`} style={this.getPosition()}><div className="dante-menu-linkinput"><input className="dante-menu-input" ref="dante_menu_input" placeholder="Paste or type a link" onKeyPress={this.handleInputEnter} defaultValue={this.getDefaultValue()} /><div className="dante-menu-button" onMouseDown={this._disableLinkMode} /></div><ul className="dante-menu-buttons">{this.blockItems().map((item, i) => {
          return <DanteTooltipItem key={i} item={item} handleClick={this._clickBlockHandler} editorState={this.props.editorState} type="block" currentStyle={this.props.editorState.getCurrentInlineStyle} />;
        })}<DanteTooltipLink editorState={this.props.editorState} enableLinkMode={this._enableLinkMode} />{this.inlineItems().map((item, i) => {
          return <DanteTooltipItem key={i} item={item} type="inline" editorState={this.props.editorState} handleClick={this._clickInlineHandler} />;
        })}</ul></div>;
  }
}
...
class DanteTooltipItem extends React.Component {
  ...
  render() {
    return <li className={`dante-menu-button ${ this.activeClass() }`} onMouseDown={this.handleClick}><i className={`dante-icon dante-icon-${ this.props.item.label }`} data-action="bold" /></li>;
  }
}
...
class DanteTooltipLink extends React.Component {
  ...
  render() {
    return <li className="dante-menu-button" onMouseDown={this.promptForLink}><i className="dante-icon icon-createlink" data-action="createlink">link</i></li>;
  }
}
