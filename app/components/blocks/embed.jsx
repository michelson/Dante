...
export default class EmbedBlock extends React.Component {
  ...
  render() {
    ...
    return <span>{this.picture() ? <a target='_blank' className={`js-mixtapeImage mixtapeImage ${ this.classForImage() }`} href={this.state.embed_data.url} style={{ backgroundImage: `url('${ this.picture() }')` }} /> : undefined}{this.state.error ? <h2>{this.state.error}</h2> : undefined}<a className='markup--anchor markup--mixtapeEmbed-anchor' target='_blank' href={this.state.embed_data.url}><strong className='markup--strong markup--mixtapeEmbed-strong'>{this.state.embed_data.title}</strong><em className='markup--em markup--mixtapeEmbed-em'>{this.state.embed_data.description}</em></a>{this.state.embed_data.provider_url}</span>;
  }
}
